...
pub async fn cmd_guess(bot: &Bot, msg: &Message, args: &str, state: &SharedState) {
    ...
    let player_id = match msg.from.as_ref() { Some(u) => u.id, None => return };
    let player_name = msg.from.as_ref().map(|u| u.first_name.clone()).unwrap_or_else(|| "Player".into());
    ...
    if args.trim().is_empty() {
        ...
        st.games.insert(chat_id, NumberGame {
            secret,
            attempts: 0,
            max_attempts: 7,
        ...
        let _ = bot.send_message(
            ...
            format!(
                ...
                You have *7 attempts*. Go! Use /guess <number>",
            ),
    ...
    let guess: u32 = match args.trim().parse() {
        Ok(n) => n,
        Err(_) => {
            let _ = bot.send_message(chat_id, "❌ Please enter a valid number! Example: /guess 42", None).await;
            return;
        }
    };

    if guess < 1 || guess > 100 {
        let _ = bot.send_message(chat_id, "❌ Number must be between 1 and 100!", None).await;
        return;
    }
    ...
    let result = {
        ...
        let game = match st.games.get_mut(&chat_id) {
            ...
            None => {
                drop(st);
                let _ = bot.send_message(chat_id, "❌ No game running! Start one with /guess", None).await;
                return;
            }
        };
        game.attempts += 1;
...
pub async fn cmd_giveup(bot: &Bot, msg: &Message, state: &SharedState) {
    ...
    match st.games.remove(&chat_id) {
        ...
        None => {
            let _ = bot.send_message(chat_id, "❌ No game is currently running.", None).await;
        }
    }
}
